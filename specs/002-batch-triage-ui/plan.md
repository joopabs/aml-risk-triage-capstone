...
**Testing**: pytest. API: `tests/api/test_batch_api.py` with the existing session-scoped synthetic
`api_bundle` fixture and FastAPI `TestClient`. UI: `tests/ui/` using `streamlit.testing.v1.AppTest`
...
deployment/ui/example_batch.csv    # small synthetic example (FR-005)
deployment/ui/demo_ui.gif          # demo capture (M3)
tests/
